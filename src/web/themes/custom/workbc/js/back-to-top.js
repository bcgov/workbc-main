(function (Drupal, $, once) {
  ...
  let backToTopId = null;

  const scrollToTop = () => {
    let backToElement = null;
    if (backToTopId && (backToElement = document.getElementById(backToTopId))) {
      backToElement.scrollIntoView(true);
    }
    else {
  ...
  Drupal.behaviors.back_to_top = {
    attach: function (context, settings) {
      if ('back_to_top' in settings) {
        backToTopId = settings.back_to_top;
      }
