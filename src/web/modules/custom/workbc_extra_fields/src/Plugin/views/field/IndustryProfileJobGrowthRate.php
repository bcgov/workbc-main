...
class IndustryProfileJobGrowthRate extends FieldPluginBase {
  ...
  public function render(ResultRow $values) {
    ...
    if ($node->bundle() == "industry_profile") {
      if (!empty($values->ssot_data) && isset($values->ssot_data['labour_force_survey_industry']['yoy_employment_growth_pct_2019_2020'])) {
        $output = ssotFormatNumber($values->ssot_data['labour_force_survey_industry']['yoy_employment_growth_pct_2019_2020'], 1, true) . '%';
      }
      else {
