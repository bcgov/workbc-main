 ...
 *
 * @SearchApiProcessor(
 *   id = "career_description_processor",
 *   label = @Translation("Career Description Processor"),
 *   description = @Translation("Pulls Career Description data from an external API and indexes it."),
 *   stages = {
 *     "add_properties" = 0,
 *     "preprocess_index" = -10
 *   }
 * )
 */
class CareerDescriptionProcessor extends ProcessorPluginBase {
  ...
  public function getPropertyDefinitions(?DatasourceInterface $datasource = NULL) {
    ...
    if (!$datasource) {
      $definition = [
        'label' => $this->t('Career Description'),
        'description' => $this->t('An Career Description field fetched from the Career Trek API.'),
        'type' => 'string',
        'processor_id' => $this->getPluginId(),
      ];
      $properties['career_description'] = new CustomValueProperty($definition);
    }
  ...
  public function addFieldValues(ItemInterface $item) {
    $fields = $this->getFieldsHelper()
      ->filterForPropertyPath($item->getFields(), NULL, 'career_description');
  ...
  protected function fetchDataFromApi($id) {
    try {
      ...
      if (!empty($ssot['career_trek'][0])) {
        return $ssot['career_trek'][0]['description'] ?? '';
      }
    }
    catch (\Exception $e) {
