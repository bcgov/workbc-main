 ...
 *
 * @SearchApiProcessor(
 *   id = "youtube_url_processor",
 *   label = @Translation("Youtube Url Processor"),
 *   description = @Translation("Pulls Youtube Url data from an external API and indexes it."),
 *   stages = {
 *     "add_properties" = 0,
 *     "preprocess_index" = -10
 *   }
 * )
 */
class YoutubeUrlProcessor extends ProcessorPluginBase {
  ...
  public function getPropertyDefinitions(?DatasourceInterface $datasource = NULL) {
    ...
    if (!$datasource) {
      $definition = [
        'label' => $this->t('Youtube Url'),
        'description' => $this->t('An Youtube Url field fetched from the Career Trek API.'),
        'type' => 'string',
        'processor_id' => $this->getPluginId(),
      ];
      $properties['youtube_url'] = new CustomValueProperty($definition);
    }
  ...
  public function addFieldValues(ItemInterface $item) {
    $fields = $this->getFieldsHelper()
      ->filterForPropertyPath($item->getFields(), NULL, 'youtube_url');
  ...
  protected function fetchDataFromApi($id) {
    try {
      ...
      if (!empty($ssot['career_trek'][0])) {
        return $ssot['career_trek'][0]['youtube_link'] ?? '';
      }
    }
    catch (\Exception $e) {
