 ...
 */
class CareerCategoryGridForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'workbc_custom_career_category_grid_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $items = NULL) {
    ...
    foreach ($items as $key => $item) {
      ...
      $category_id = $paragraph->field_career_category->target_id;
      ...
      if (isset($category_id)) {
        ...
        $form[$category_label]['help'] = [
          '#markup' => '<div class="areas-of-interest-help">' . $this->t('Select one or more areas of interest within @category.', ['@category' => strtolower($category->getName())]) . '</div>',
        ];
        ...
        $form[$category_label][$category->tid->value] = [
          '#type' => 'checkbox',
        ...
        foreach ($areas as $area) {
          $form[$category_label][$area->tid->value] = [
            '#type' => 'checkbox',
            '#title' => $area->getName(),
        ...
        $form[$category_label]['submit'] = [
          ...
          '#suffix' => '<span class="error hidden"><span class="error-group"><span class="error-icon"></span><span class="error-text">' . $this->t('Choose one or more options within @category', ['@category' => $category->getName()]) . '</span></span></span></div>',
        ];
        ...
        $tile = '<div class="tile">';
        $tile .= '<div class="tile-info tile-name">Explore Your Career Options</div>';
        $tile .= '<div class="tile-info tile-arrow">';
        $tile .= '<a href="' . $paths['career_exploration'] . '">';
        $tile .= '<img src="/modules/custom/workbc_custom/icons/arrow-right.svg" title="right arrow"/>';
        $tile .= '</a>';
        $tile .= '</div>';
        $tile .= '</div>';
