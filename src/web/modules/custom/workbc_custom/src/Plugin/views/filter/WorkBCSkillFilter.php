...
// based on web\core\modules\taxonomy\src\Plugin\views\filter\TaxonomyIndexTid.php
 ...
 *
 * @ViewsFilter("workbc_taxonomy_index_tid_filter")
 */
class WorkBCSkillFilter extends ManyToOne {
  ...
  public function buildExtraOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    $form['top'] = [
      '#type' => 'number',
      '#title' => $this->t('Number of top Skills to filter on'),
      // '#options' => ['select' => $this->t('Dropdown'), 'textfield' => $this->t('Autocomplete')],
      '#default_value' => $this->options['top'],
    ];
  ...
  public function query() {
    ...
    if ($this->options['exposed']) {
      ...
      $table = $this->relationship . "__" . $this->options['table'];
      $query = $this->query;
      $query->addWhere($this->options['group'], $table . '.' . $this->options['field'], $this->value[0], '=');
      if ($this->options['top'] > 0) {
        $query->addWhere($this->options['group'], $table . '.delta', $this->options['top'], '<');
      }
    }
  }
